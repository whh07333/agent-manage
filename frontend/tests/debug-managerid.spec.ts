...
test.describe('Debug managerId fix', () => {
  test.use({
    storageState: {
      origins: [
        {
  ...
  test('debug after managerId fix', async ({ page }) => {
    ...
    // What's the current priority buttons
    const buttons = await page.locator('.ant-input-group button').allTextContents();
    console.log('Priority buttons:', buttons);
