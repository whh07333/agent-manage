...
test.describe('Debug priority fix', () => {
  test.use({
    storageState: {
      origins: [
        {
  ...
  test('debug create project after priority fix', async ({ page }) => {
    ...
    // What's the button text now?
    const buttons = await page.locator('.ant-modal button').allTextContents();
